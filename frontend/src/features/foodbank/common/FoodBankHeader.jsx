...
const FoodBankHeader = () => {
  ...
  const foodBankName = auth?.user?.name || "SharePlate Food Bank";

  const navLinks = useMemo(
    () => [
      {
        name: "Browse Food",
        path: "/foodbank/donated-food",
        icon: Search,
      },
      {
        name: "Broadcast Need",
        path: "/foodbank/post-request",
        icon: Megaphone,
      },
      {
        name: "My Broadcasts",
        path: "/foodbank/my-proactive-requests",
        icon: Inbox,
      },
      {
        name: "Complaints",
        path: "/foodbank/complaints",
        icon: MessageSquare,
      },
      {
        name: "Profile",
    ...
    <header className="sticky top-0 z-50 border-b border-emerald-100/80 bg-white/90 backdrop-blur-xl supports-[backdrop-filter]:bg-white/75">
      <div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link
          ...
          aria-label="Go to food bank dashboard"
        >
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          <button
            ...
            aria-controls="foodbank-mobile-navigation"
...
export default React.memo(FoodBankHeader);
